...
app.controller('ctrlStockAgeBand', ['$stateParams', '$state', '$http', '$scope', '$rootScope', 'GlobalHelper', 'HttpErrorService', 'HttpSuccessService',
function ($stateParams, $state, $http, $scope, $rootScope, GlobalHelper, Notification, HttpErrorService, $AspCookie) {
    var self = this;
    self.listStockAge = true;
    self.itemsPerPage = 8;
    self.currentPage = 1;
    self.isTrade = false;
    ...
    var clStockAge = function () {
        return {
            "ID": null,
            "STOCKAGE_BAND_DESC": null,
            "STKAGE_PROD_LEVEL1_CODE": null,
            "STOCKAGE_BAND_LOWER": null,
            "STOCKAGE_BAND_UPPER": null,
            "STOCKAGE_BAND_SEQ": null,
            "LATEST": null,
            "ARC_DATE": null,
    ...
    self.onInit = function (ipage) {
        ...
        $http.get(biApi + 'StockAge/getStockAge?&pg=' + self.currentPage + '&tk=' + self.itemsPerPage + '&fnd=' + $rootScope.search).success(function (response, status, headers, config) {
            console.log(response);
            if (!response.hasError) {
                //self.console.log;
                self.StockAgeList = response.data;
    ...
    self.onEditMst = function (StockAge) {
        //self.disabledUserName = true;
        self.viewOnly = false;
        self.btnSave = false;
        self.btnUpdate = true;
        self.objStockAge.ID = StockAge.ID;
        self.objStockAge.STOCKAGE_BAND_DESC = StockAge.STOCKAGE_BAND_DESC;
        self.objStockAge.STKAGE_PROD_LEVEL1_CODE = StockAge.STKAGE_PROD_LEVEL1_CODE;
        self.objStockAge.STOCKAGE_BAND_LOWER = StockAge.STOCKAGE_BAND_LOWER;
        self.objStockAge.STOCKAGE_BAND_UPPER = StockAge.STOCKAGE_BAND_UPPER;
        self.objStockAge.STOCKAGE_BAND_SEQ = StockAge.STOCKAGE_BAND_SEQ;
        self.objStockAge.LATEST = StockAge.LATEST;
        self.objStockAge.ARC_DATE = StockAge.ARC_DATE;
    ...
    self.StockAgeSearch = function (pCode) {
        self.objStockAge.lCode = self.objStockAge.lCode
        //$rootScope.brandCOde = self.pcode;
        self.processSearch(self.currentPage);
    }

    self.processSearch = function () {
        if (self.objStockAge.lCode === null) {
            GlobalHelper.Error("Level Code is required");
            document.getElementById("idSite").focus();
            return;
        }

        $http.get(biApi + 'StockAge/getStockAgeByCode?&pg=' + self.currentPage + '&tk=' + self.itemsPerPage + '&fnd=' + $rootScope.search + '&pCode=' + self.objStockAge.lCode)
        .success(function (response, status, headers, config) {
            console.log(response.data);
            if (!response.hasError) {

                self.StockAgeList1 = response.data;
                self.totalstock = response.totalCount;
    ...
    self.onView = function () {
        self.viewOnly = false;
        self.listStockAge = false;
        self.btnSave = true;
        self.btnUpdate = false;
        self.objStockAge = new clStockAge();
